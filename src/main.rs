...
"#,
    ...
    match Program::compile(&code) {
        ...
        Ok(prgm) => {
            ...
            let mut env = Environment::new(&mut memory, &prgm, &input[..], &mut output[..]);
            let err = env.run().err();
            if err.is_some() {
                dprintln!(" ERR");
            }
            let (o, e) = match str::from_utf8(&output) {
                Ok(o) => (o, None),
                Err(e) => (
                    // SAFETY: `valid_up_to` is by definition the length of the string that is valid UTF-8.
                    unsafe { str::from_utf8_unchecked(&output[..e.valid_up_to()]) },
                    Some(e),
                ),
            };
